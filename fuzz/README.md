...
The corpus contains test inputs that achieve good code coverage. Libfuzzer automatically generates and saves interesting inputs as it discovers new code paths.

```bash
# View corpus (auto-generated by fuzzer)
...
**Note**: This fuzzer uses `arbitrary::Arbitrary` for structured input generation, so corpus files are binary-serialized data structures, not human-readable text.
