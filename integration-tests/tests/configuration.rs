...
//!
//! This module contains integration tests for AWS Workload Credentials Provider's configuration features.
//! These tests verify that configuration options work correctly and affect provider behavior as expected,
...
async fn test_ping_endpoint_health_check() {
    ...
    let response_no_token = provider.make_ping_request().await;
    ...
    let response_with_token = provider.make_ping_request_with_token().await;
...
async fn test_path_based_requests() {
    ...
    let response_path = provider.make_path_based_request(&secret_name).await;
    ...
    let query = ProviderQueryBuilder::default()
        ...
        .build()
        .unwrap();
    let response_query = provider.make_request_raw(&query).await;
